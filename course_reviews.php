<?php // course_reviews.php - Course Reviews Page ?>
        ...
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <div class="w-full md:w-3/4 lg:w-4/5">
                <div class="flex flex-col sm:flex-row justify-between items-center mb-8">
                    ...
                    <button class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-5 rounded-lg shadow-md transition duration-300 flex items-center">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                        <div class="md:col-span-1">
                            ...
                            <div class="relative">
                                ...
                                <input type="search" id="search-reviews" class="w-full p-2 pl-10 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500" placeholder="e.g., CS 101 or Prof. Smith">
                            </div>
                        </div>
                        <div>
                ...
                <div class="space-y-6">
                    ...
                    function generate_stars($rating, $max_stars = 5) {
                        $full_star_svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 text-yellow-400"><path fill-rule="evenodd" d="M10.868 2.884c.321-.772 1.415-.772 1.736 0l1.681 4.06c.064.155.195.278.358.325l4.422.644c.828.121 1.164 1.132.533 1.72l-3.209 3.126a.427.427 0 00-.122.38l.758 4.398c.139.813-.71.1.443-.386l-3.952-2.078a.427.427 0 00-.401 0l-3.952 2.078c-.63.33-.982-.033-.843-.843l.758-4.398a.427.427 0 00-.122-.38L2.613 9.753c-.63-.613-.295-1.599.533-1.72l4.422-.644a.427.427 0 00.358-.325l1.681-4.06z" clip-rule="evenodd" /></svg>';
                        $empty_star_svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 text-gray-300"><path fill-rule="evenodd" d="M10.868 2.884c.321-.772 1.415-.772 1.736 0l1.681 4.06c.064.155.195.278.358.325l4.422.644c.828.121 1.164 1.132.533 1.72l-3.209 3.126a.427.427 0 00-.122.38l.758 4.398c.139.813-.71.1.443-.386l-3.952-2.078a.427.427 0 00-.401 0l-3.952 2.078c-.63.33-.982-.033-.843-.843l.758-4.398a.427.427 0 00-.122-.38L2.613 9.753c-.63-.613-.295-1.599.533-1.72l4.422-.644a.427.427 0 00.358-.325l1.681-4.06z" clip-rule="evenodd" /></svg>';
                        $stars_html = '';
                        for ($i = 1; $i <= $max_stars; $i++) {
                            $stars_html .= ($i <= round($rating)) ? $full_star_svg : $empty_star_svg;
                        }
                        return $stars_html;
                    }

                    // Placeholder course review data
                    $course_reviews = [
                        [
                            ...
                            'overall_rating' => 4.8,
                            'review_snippet' => '"Great intro course! Prof. Smith makes complex concepts easy to understand and the assignments were challenging but fair."',
                            'reviewer_name' => 'Thomas L.',
                            'reviewer_term' => 'Spring 2023',
                            'individual_rating' => 5.0,
                            'total_reviews' => 28,
                            'take_again_percentage' => 94,
                            'bg_color' => 'bg-white' // Or specific colors like 'bg-green-50'
                        ],
                        [
                            ...
                            'overall_rating' => 3.5,
                            'review_snippet' => '"Challenging course but very rewarding if you put in the work. Dr. Johnson\'s lectures can be hard to follow sometimes."',
                            'reviewer_name' => 'Alex W.',
                            'reviewer_term' => 'Fall 2023',
                            'individual_rating' => 3.0, // Adjusted to match 3.5 overall better
                            'total_reviews' => 42,
                            'take_again_percentage' => 68,
                            'bg_color' => 'bg-white'
                        ],
                        [
                            ...
                            'overall_rating' => 4.2,
                            'review_snippet' => '"Prof. Williams is passionate about history and makes every lecture engaging. Lots of reading but all of it was interesting."',
                            'reviewer_name' => 'Jamie R.',
                            'reviewer_term' => 'Spring 2024',
                            'individual_rating' => 4.0,
                            'total_reviews' => 35,
                            'take_again_percentage' => 89,
                            'bg_color' => 'bg-white'
                        ],
                    ];

                    foreach ($course_reviews as $review): ?>
                        <div class="course-review-card <?php echo htmlspecialchars($review['bg_color']); ?> p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300">
                            <div class="flex flex-col sm:flex-row justify-between items-start mb-3">
                                <div>
                                    <h3 class="text-xl font-semibold text-gray-800"><?php echo htmlspecialchars($review['title']); ?></h3>
                                    <p class="text-sm text-gray-600"><?php echo htmlspecialchars($review['course_code']); ?> &bull; <?php echo htmlspecialchars($review['professor']); ?></p>
                                </div>
                                <div class="mt-2 sm:mt-0 bg-green-100 text-green-700 font-bold text-sm px-3 py-1 rounded-full">
                                    <?php echo htmlspecialchars(number_format($review['overall_rating'], 1)); ?> / 5
                                </div>
                            </div>
                            <blockquote class="text-gray-700 italic text-sm border-l-4 border-gray-200 pl-4 py-2 my-3">
                                <?php echo htmlspecialchars($review['review_snippet']); ?>
                            </blockquote>
                            <div class="flex flex-col sm:flex-row justify-between items-start text-sm text-gray-600 mb-3">
                                <span><?php echo htmlspecialchars($review['reviewer_name']); ?> &bull; <?php echo htmlspecialchars($review['reviewer_term']); ?></span>
                                <div class="flex items-center mt-1 sm:mt-0">
                                    <?php echo generate_stars($review['individual_rating']); ?>
                                    <span class="ml-1">(<?php echo htmlspecialchars(number_format($review['individual_rating'], 1)); ?>)</span>
                                </div>
                            </div>
                            <div class="flex justify-between items-center text-xs text-gray-500">
                                <span><?php echo htmlspecialchars($review['total_reviews']); ?> reviews</span>
                                <span class="font-semibold text-green-600"><?php echo htmlspecialchars($review['take_again_percentage']); ?>% would take again</span>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
                ...
                <div class="mt-12 flex justify-center items-center space-x-2">
                    ...
                    <a href="#" class="px-4 py-2 rounded-md hover:bg-gray-200 text-gray-700">12</a>
                    <a href="#" class="p-2 rounded-md hover:bg-gray-200 text-gray-600">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
...
<script>
    // Any specific JS for the course reviews page can go here
    // For example, handling search/filter interactions or "Write a Review" modal
</script>
</body>
</html>
